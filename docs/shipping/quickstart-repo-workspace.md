# Quickstart — dépôt + workspace agent
...
2. `git clone …` puis `cd french-childcare-costs` et `bun install`.
3. (Optionnel) Lier le skill : `ln -sf "$(pwd)/harness/skill" ~/.claude/skills/comparatif-modes-garde-fr-2026` — vérifier le chemin des skills sur la doc de ton IDE agent (ex. Anthropic : [docs.anthropic.com](https://docs.anthropic.com)).
