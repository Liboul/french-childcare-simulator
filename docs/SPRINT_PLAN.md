...
| **GARDE-016** | 2026-03-21       | Done. `harness/` : `POST /v1/calculate` (Bun), `openapi.yaml`, instructions GPT + prompts, `skill/SKILL.md`, test `harness-calculate`.                                                                                                                                                                                                                    |
...
| **GARDE-027** | 2026-03-21       | Done. `docs/shipping/quickstart-*.md` (repo workspace, ZIP harness skill, GPT, Gemini).                                                                                                                                                                                                                                                                   |
...
| **GARDE-031** | 2026-03-21       | Done. CI `package:harness-skill` + `unzip -l` (SKILL, INTAKE, schema, examples).                                                                                                                                                                                                                                                                          |
| **GARDE-035** | 2026-03-21       | Done. Skill ZIP : `scripts/simulate.mjs` (bundle Node), SKILL incite exécution obligatoire ; **sans** `openapi.yaml` dans l’archive ; CI smoke `node …/simulate.mjs`. Spec : `docs/stories/GARDE-035.md`.                                                                                                                                                 |
...
#### DR-05 — Provider harnesses (Agent Skills, OpenAI, Gemini — how to ship)
...
| **Harness** | Instructions, tool wiring, optional wrappers so an agent _uses_ the core (Agent Skills, ChatGPT, Gemini, …) | **Per-vendor** artifacts + docs           |
...
| **GARDE-015** | `[SPIKE]` **`[DEEP RESEARCH]`** Provider harness & distribution: DR-05 + ADR (`docs/architecture/` or `docs/research/`)               | E4    | **Clarifies Agent Skills vs OpenAI vs Gemini**; defines what “ship” means before GARDE-016         |
...
| **GARDE-018** | **Shipping runbooks** : `docs/shipping/` (Anthropic / OpenAI / Gemini), checklists + liens repo + DR-05 ; dépend de **GARDE-016**     | E4    | Pas d’hébergement managé dans la story ; auth/TLS à la charge du déployeur                         |
...
| **GARDE-024** | **Packaging skill** : profils « repo + Bun » vs « ZIP + HTTP » documentés ; script / doc alignés                                      | E4    | Étend **GARDE-018** / `package:harness-skill`                                                      |
...
| **GARDE-027** | **Quickstarts** : une page courte par canal sous `docs/shipping/` (repo workspace, ZIP harness skill, GPT, Gemini)                    | E4    | Captures / liens doc fournisseur « au moment T »                                                   |
...
| **GARDE-031** | **CI : artefact skill** — `package:harness-skill` en CI + assertion sur le contenu du ZIP                                             | E0    | Empêche régressions de packaging                                                                   |
