...
- **CESU préfinancé (emploi à domicile)** : avec `taxCredit.prefundedCesuAnnualEur` > 0 pour `nounou_domicile` / `nounou_partagee`, une étape **`trace`** `scenario_tax_credit_prefunded_cesu` décrit l’effet sur l’assiette ; plafonds employeur aussi dans `warnings` si besoin. **Avant** saisie : **`INTAKE.md`** § 3 bis **partie A** (question **obligatoire**, puis complément vs substitution vs brut URSSAF).
